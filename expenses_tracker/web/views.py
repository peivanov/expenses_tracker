...
def get_profile():
    ...
    if profiles:
        return profiles[0]  # if profile get me the first one
    return None
...
def create_profile(request):
    ...
    if request.method == 'POST':
        form = CreateProfileForm(request.POST, request.FILES)   # request.FILES - за снимки
        if form.is_valid():
            form.save()
            return redirect('show index')
    else:
...
def edit_profile(request):
    profile = get_profile()     # <---
    if request.method == 'POST':
        form = EditProfileForm(request.POST, request.FILES, instance=profile)  # <--- instance при edit
        if form.is_valid():
            form.save()
            return redirect('show index')
    else:
        form = EditProfileForm(instance=profile)    # <--- instance

    context = {
        'form': form,
        'no_profile': True,     # if no profile, don`t show profile button
    }
...
def delete_profile(request):
    profile = get_profile()  # <---
    if request.method == 'POST':
        form = DeleteProfileForm(request.POST, request.FILES, instance=profile)  # <--- instance при edit
        if form.is_valid():
            form.save()
            return redirect('show index')
    else:
        form = DeleteProfileForm(instance=profile)  # <--- instance
